...
import { Play, RotateCcw, Save, Trash2, Check, TrendingUp, Info } from 'lucide-react';
...
export default function Calculator() {
  ...
  // 실시간 DPS 재계산 트리거
  useEffect(() => {
    // selectedRunes를 1차원 리스트로 플래시하여 calculator에 보냄
    ...
    const result = calculateDPS(stats, flattenedRunes, gimmicks, cycles, conditionalUptimes);
    setDpsResult(result);
  }, [stats, selectedRunes, gimmicks, cycles, conditionalUptimes]);

  // 로컬 스토리지 프리셋 로드
  useEffect(() => {
    const saved = localStorage.getItem('mabi_runes_presets');
    if (saved) {
      try {
  ...
  const handleReset = () => {
    if (window.confirm('모든 능력치와 룬 설정을 초기화하시겠습니까?')) {
      setStats({
        baseAttack: 27166.0,
        critScore: 6925.0,
        strongDmg: 2487.0,
  ...
  const savePreset = (slotIndex) => {
    ...
    localStorage.setItem('mabi_runes_presets', JSON.stringify(newPresets));
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div>
          <h2 className="text-2xl font-black text-slate-100 flex items-center gap-2">
            ...
            <span className="text-xs bg-slate-800 border border-slate-700 px-2 py-0.5 rounded-full text-slate-400 font-bold">시즌 2 개편 대응</span>
          </h2>
          <p className="text-xs text-slate-400 mt-1 leading-relaxed">
            시즌 2에서 완전히 개편된 신규 룬 데이터베이스와 조건부 가동률 시뮬레이션을 탑재한 DPS 연산기입니다.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleReset}
            className="flex items-center gap-1.5 px-4 py-2 bg-slate-850 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-semibold text-slate-350 active:scale-95 transition-all"
          >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            ...
            <div className="flex flex-col gap-3">
              ...
              <span className="text-[9px] text-slate-500 leading-normal mt-1 flex items-center gap-1">
                ...
                스킬 입력 약어: 1 (차징피스트 1타/2타), 2 (연환격 1타/2타), 3 (섬격), 4 (필사의 일격 1타/2타), 5 (궁극기)
              </span>
            </div>
