...
export default function ConditionalSettingsSidebar({
  ...
  return (
    <>
      <details className="mb-1 rounded-xl border border-theme bg-theme-card theme-transition lg:hidden">
        <summary className="flex cursor-pointer list-none items-center justify-between px-3 py-2.5 text-xs font-black text-theme-main">
          <span className="flex items-center gap-2"><SlidersHorizontal className="h-4 w-4 text-orange-500" />조건부 버프·가동률 조정</span>
      ...
      >
        ...
        <span className="[writing-mode:vertical-rl] text-[10px] font-black tracking-[0.12em]">조건부 설정</span>
      </button>

      {isOpen && (
        <button
          type="button"
          aria-label="조건부 설정 패널 닫기"
          onClick={onClose}
      ...
      <aside
        ...
        aria-label="조건부 버프 및 가동률 설정"
        aria-hidden={!isOpen}
        className={`fixed left-0 top-0 z-50 hidden h-dvh w-[360px] max-w-[calc(100vw-1rem)] border-r border-theme bg-theme-main shadow-2xl transition-transform duration-300 ease-out lg:flex lg:flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full pointer-events-none'}`}
      >
        <div className="flex items-center justify-between border-b border-theme bg-theme-card px-4 py-3 theme-transition">
          <div>
            <p className="text-[10px] font-black tracking-[0.14em] text-orange-500">OPTIONAL SETTINGS</p>
            <h2 className="mt-0.5 text-sm font-black text-theme-main">조건부 버프·가동률</h2>
          </div>
          <button
            type="button"
            aria-label="조건부 설정 닫기"
            onClick={onClose}
