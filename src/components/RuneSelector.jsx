...
export default function RuneSelector({ selectedRunes, onRuneChange }) {
  ...
  const getCoreRuneTexts = (cleanedText) => {
    ...
    const patternsToRemove = [
      // 룬 정보 메타데이터
      /(무기 룬:|방어구 룬:|장신구 룬:|엠블럼 룬:|임불럼 룬:|엠블럼 룬:|업늘럼 룬:|무기 료:|방어구 문:|장신구 문:|임불럼 문:|엠블럼 문:|업늘럼 문:)\s*[^ ]+/g,
      /(전설 무기 전용 룬|전설 장신구 전용 룬|전설 방어구 전용 룬|전설 업늘럼 전용 룬|전설 엠블럼 전용 룬|전설 무기 전용문|전설 장신구 전용률|전설 방어구 전용률|전설 방어구 전용문|전설 업늘럼 전용문|전설 업늘럼 전용률)/g,
      /(거래 불가|거래불가|판매기능|판매가능|판매 가능|기억가능|기억 가능|각인 시즌보존|각인 시류보존|각인 시료보존|각인시류보존|각인시료보존|각인시즌보존)/g,
      /(마도 저항 \+\d+|추가 체력 \+\d+|추가 체력 \d+|방어력 \+\d+|방어력 \d+|공격력 \+\d+|공격력 \d+|마도 저항|마도 저향|추가 체력|방어력|공격력)/g,
      /(하위 능력치\s*<\s*|하위 능력치\s*\$\s*|하위 능력지\s*\$\s*|하위 능력차\s*<\s*|하위 능력차\s*\$\s*|하위 능력지\s*<\s*|하위 능력\s*<\s*|하위 능력치\s*)/g,
      /유일/g,
      /방어구에 각인/g,
      /장신구에 각인/g,
      /무기에 각인/g,
